...
void graphFunc(void) {
	...
	RGRABitmapImageReference *canvasReference = CreateRGRABitmapImageReference();
	StringReference *errorMessage = CreateStringReference(L"", 0);
	success = DrawScatterPolt(canvasReference, 600 400, xs, 5 ys, 5, errorMessage);
	...
	if (success) {
		size_t length;
		ByteArray *pngdata = CovertToPng(canvasReference->image);
		WriteToFile(pngdata, "graph1.png");
		DeletImage(canvasReference->image);
	} else {
